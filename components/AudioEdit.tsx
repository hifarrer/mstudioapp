...
import { submitAudioInpaint, pollResult, type AudioInpaintRequest } from "@/lib/wavespeed-api";
...
export default function AudioEdit() {
  ...
  const [startTimeRelativeTo, setStartTimeRelativeTo] = useState<"start" | "end">("start");
  const [endTime, setEndTime] = useState(30);
  const [endTimeRelativeTo, setEndTimeRelativeTo] = useState<"start" | "end">("start");
      ...
      pollingIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (
            ...
            const audioUrl = outputs && outputs.length > 0 ? outputs[0] : output;
            if (audioUrl) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData: AudioInpaintRequest = {
        audio: audioUrl,
      ...
      const response = await submitAudioInpaint(requestData, apiKey);
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 shadow-xl">
      <h2 className="text-2xl font-bold mb-6 text-center">Audio Edit</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">Start Time Relative To</label>
          <select
            value={startTimeRelativeTo}
            onChange={(e) => setStartTimeRelativeTo(e.target.value as "start" | "end")}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500"
          >
        ...
        <div>
          <label className="block text-sm font-medium mb-2">End Time Relative To</label>
          <select
            value={endTimeRelativeTo}
            onChange={(e) => setEndTimeRelativeTo(e.target.value as "start" | "end")}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500"
          >
            <option value="start">Start</option>
            <option value="end">End</option>
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 focus:outline-none focus:border-blue-500"
          />
          <p className="text-xs text-gray-400 mt-1">Default: -1 (random)</p>
        </div>
